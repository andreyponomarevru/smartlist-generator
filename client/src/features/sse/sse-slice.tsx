...
import { SSEMessage, OSProcessName, LibPathInput } from "../../types";
...
export const extendedAPIslice = apiSplitSlice.injectEndpoints({
  endpoints: (builder) => ({
    streamSSEs: builder.query<SSEMessage[], OSProcessName>({
    ...
    startProcess: builder.mutation<
      void,
      { processName: OSProcessName } & LibPathInput
    >({
      query: (arg) => ({
    ...
    stopProcess: builder.mutation<void, { processName: OSProcessName }>({
      query: (arg) => ({
        url: `/processes/${arg.processName}`,
        method: "DELETE",
      }),
