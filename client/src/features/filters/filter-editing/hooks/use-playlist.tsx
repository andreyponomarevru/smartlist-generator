...
export function usePlaylist() {
  ...
  function handleTracksReset() {
    dispatch({ type: "RESET" });
  }

  async function handleTrackReplace({ trackId, formValues }: TrackToReplace) {
  ...
  async function handleTrackAdd(formValues: FilterFormValues) {
    const excludedTracks = Object.values(state.tracks)
      .flat()
      .map((t) => t.trackId);
  ...
  function handleTrackReorder({ index, direction }: TrackToReorder) {
